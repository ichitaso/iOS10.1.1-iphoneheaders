/*
* This header is generated by classdump-dyld 1.0
* on Tuesday, December 27, 2016 at 4:44:18 PM Japan Standard Time
* Operating System: Version 10.1.1 (Build 14B150)
* Image Source: /System/Library/VideoProcessors/StereoFusion.videoprocessor
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/


@class PyramidStorage, RegPyrFusionResources, MainFusionResources;

@interface SFBuffers : NSObject {

	PyramidStorage* refPyramid;
	PyramidStorage* nonRefPyramid;
	RegPyrFusionResources* regPyrFusionResources;
	MainFusionResources* mainFusionResources;

}
-(id)init;
-(void)releaseBuffers;
@end

