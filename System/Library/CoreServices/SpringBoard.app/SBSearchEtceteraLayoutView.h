/*
* This header is generated by classdump-dyld 1.0
* on Tuesday, December 27, 2016 at 4:49:46 PM Japan Standard Time
* Operating System: Version 10.1.1 (Build 14B150)
* Image Source: /System/Library/CoreServices/SpringBoard.app/SpringBoard
* classdump-dyld is licensed under GPLv3, Copyright © 2013-2016 by Elias Limneos.
*/

#import <SpringBoard/SpringBoard-Structs.h>
#import <UIKit/UIView.h>
#import <SpringBoard/BSUIScrollViewDelegate.h>

@protocol SBSearchEtceteraLayoutViewDelegate;
@class SBSearchEtceteraTodayLayoutContentView, SBSearchEtceteraSearchLayoutContentView, SBSearchEtceteraNotificationsLayoutContentView, SBSearchEtceteraLayoutContentView, SBPagedScrollView, UIView, UIPageControl, NSString;

@interface SBSearchEtceteraLayoutView : UIView <BSUIScrollViewDelegate> {

	SBSearchEtceteraTodayLayoutContentView* _todayContentView;
	SBSearchEtceteraSearchLayoutContentView* _searchContentView;
	SBSearchEtceteraNotificationsLayoutContentView* _notificationsContentView;
	SBSearchEtceteraLayoutContentView* _visibleView;
	unsigned long long _currentMode;
	SBPagedScrollView* _scrollView;
	UIView* _notificationsPageView;
	UIView* _todayPageView;
	UIPageControl* _pageControl;
	BOOL _todayContentUnderlapsNavBar;
	unsigned long long _currentStyle;
	double _navigationBarTopInset;
	double _contentBottomInset;
	id<SBSearchEtceteraLayoutViewDelegate> _delegate;

}

@property (getter=_pageControl,nonatomic,retain,readonly) UIPageControl * pageControl;                                  //@synthesize pageControl=_pageControl - In the implementation block
@property (getter=_visibleView,nonatomic,retain,readonly) SBSearchEtceteraLayoutContentView * visibleView;              //@synthesize visibleView=_visibleView - In the implementation block
@property (nonatomic,retain) SBSearchEtceteraTodayLayoutContentView * todayContentView;                                 //@synthesize todayContentView=_todayContentView - In the implementation block
@property (nonatomic,retain) SBSearchEtceteraSearchLayoutContentView * searchContentView;                               //@synthesize searchContentView=_searchContentView - In the implementation block
@property (nonatomic,retain) SBSearchEtceteraNotificationsLayoutContentView * notificationsContentView;                 //@synthesize notificationsContentView=_notificationsContentView - In the implementation block
@property (assign,nonatomic) unsigned long long currentMode;                                                            //@synthesize currentMode=_currentMode - In the implementation block
@property (assign,nonatomic) unsigned long long currentStyle;                                                           //@synthesize currentStyle=_currentStyle - In the implementation block
@property (assign,nonatomic) BOOL todayContentUnderlapsNavBar;                                                          //@synthesize todayContentUnderlapsNavBar=_todayContentUnderlapsNavBar - In the implementation block
@property (assign,nonatomic) double navigationBarTopInset;                                                              //@synthesize navigationBarTopInset=_navigationBarTopInset - In the implementation block
@property (assign,nonatomic) double contentBottomInset;                                                                 //@synthesize contentBottomInset=_contentBottomInset - In the implementation block
@property (assign,nonatomic,__weak) id<SBSearchEtceteraLayoutViewDelegate> delegate;                                    //@synthesize delegate=_delegate - In the implementation block
@property (readonly) unsigned long long hash; 
@property (readonly) Class superclass; 
@property (copy,readonly) NSString * description; 
@property (copy,readonly) NSString * debugDescription; 
-(void)_setCurrentMode:(unsigned long long)arg1 force:(BOOL)arg2 ;
-(id)_contentViewForControllerMode:(unsigned long long)arg1 ;
-(void)_addPageControl;
-(void)_layoutPageControl;
-(void)updateViewForInitialPresentation;
-(void)setContentBottomInset:(double)arg1 ;
-(void)updateVisiblePages;
-(double)contentBottomInset;
-(BOOL)todayContentUnderlapsNavBar;
-(double)navigationBarTopInset;
-(void)_updateScrollViewVisibilityForSupportedModes;
-(void)_updateScrollingEnabledForSupportedModes;
-(void)_resetContentOffsetForSupportedModes;
-(void)setNavigationBarTopInset:(double)arg1 ;
-(unsigned long long)_currentScrollViewMode;
-(unsigned long long)_indexOfNotificationsPageView;
-(unsigned long long)_indexOfTodayPageView;
-(unsigned long long)_scrollViewModeForIndex:(unsigned long long)arg1 ;
-(BOOL)_shouldAllowScrollingForStyle:(unsigned long long)arg1 ;
-(void)_delegateCurrentModeWillChange:(unsigned long long)arg1 ;
-(void)_delegateCurrentModeDidChange:(unsigned long long)arg1 ;
-(void)setTodayContentUnderlapsNavBar:(BOOL)arg1 ;
-(unsigned long long)_nextLogicalModeIfResigningSearch;
-(SBSearchEtceteraTodayLayoutContentView *)todayContentView;
-(void)setTodayContentView:(SBSearchEtceteraTodayLayoutContentView *)arg1 ;
-(SBSearchEtceteraSearchLayoutContentView *)searchContentView;
-(void)setSearchContentView:(SBSearchEtceteraSearchLayoutContentView *)arg1 ;
-(SBSearchEtceteraNotificationsLayoutContentView *)notificationsContentView;
-(void)setNotificationsContentView:(SBSearchEtceteraNotificationsLayoutContentView *)arg1 ;
-(id)initWithFrame:(CGRect)arg1 ;
-(void)layoutSubviews;
-(void)setDelegate:(id<SBSearchEtceteraLayoutViewDelegate>)arg1 ;
-(id<SBSearchEtceteraLayoutViewDelegate>)delegate;
-(unsigned long long)currentStyle;
-(unsigned long long)currentMode;
-(id)_visibleView;
-(id)_pageControl;
-(void)setCurrentMode:(unsigned long long)arg1 ;
-(void)setCurrentStyle:(unsigned long long)arg1 ;
-(void)scrollViewDidScroll:(id)arg1 withContext:(SCD_Struct_SB34)arg2 ;
-(void)scrollViewWillBeginScrolling:(id)arg1 ;
-(void)scrollViewDidEndScrolling:(id)arg1 ;
@end

